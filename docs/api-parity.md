...
| `POST /submit {language, code}` | `POST /executions {tenant_id, language, source, ...}` | `202 {id, status: queued}`; requires `X-Tenant-ID` (`tenant_id` must match). Persisted, then published to JetStream `exec.submit.<lang>` (`502` + `failed` when the queue is down). Python only, `timeout_s` 1–60 (default 2). Per-tenant submit quotas apply (`429` with `Retry-After` when the hourly or active cap is hit). |
