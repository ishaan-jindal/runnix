...
| Legacy | v2 | Fate |
|---|---|---|
| `POST /auth/register|login|refresh|logout|logout-all` | `POST /v1/auth/register|login|refresh` | Rebuilt; access token carries `tenants[]`, refresh re-resolves memberships. Logout = client discards + server revocation list (deferred: auth-plus-postgres). |
| `GET/PATCH /auth/me`, password, api-keys CRUD | `GET /v1/users/me` + `POST /v1/tenants/:id/api-keys` (deferred: tenant-api-keys) | Scoped per tenant; user-level keys optional. |
